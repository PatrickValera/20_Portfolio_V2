...
const Projects = ({ projects }) => {
  return (
    <>
      ...
      <Box display='flex'sx={{flexWrap:'wrap',gap:2, mb: 4,minHeight:'10px',bgcolor:'purple' }}>
        {projects.slice(3).map((project, index) => (
          <Box key={index} className='fade' sx={{ p: 0, flex: '100px 1 1',minWidth:'33%', bgcolor:'primary.main' }}>
            <Typography variant='body1'>{project.title}</Typography>
            {/* <Typography variant='body1'>{project.description}</Typography> */}
