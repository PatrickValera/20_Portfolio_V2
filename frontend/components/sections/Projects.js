      ...
      {projects.slice(0, 3).map((project, index) => (
        <Fade key={index} sx={{ display: 'grid', gridTemplateColumns: 'repeat(12,1fr)', position: 'relative', mb: '10vh' }}>
          ...
          <Box sx={{ gridArea: { xs: '1/1/-1/13', md: '1/1/-1/8' }, bgcolor: 'text.disabled' }}>
            <Box sx={{ border:'2px solid ',height:'100%',borderColor:'text.primary',display:'flex',alignItems:'center'}}>
          ...
          <Box sx={{ p: 2, gridArea: { xs: '1/1/-1/13', md: '1/6/-1/-1' }, display: 'flex', flexDirection: 'column', textAlign: 'right',alignItems:'flex-end', justifyContent: 'center',bgcolor:{xs:'rgba(255,255,255,.8)',md:'unset'},zIndex:'5', }}>
            <Typography variant='h6'>{project.title}</Typography>
            <Typography variant='body2' sx={{ bgcolor: 'text.primary', color: 'background.paper', p: {xs:1,md:3}, mb: 1, border:'2px solid',borderColor:'paper.primary',maxWidth:{xs:'350px',md:'700px'} }}>{project.description}
            </Typography>
