...
export const SocialsNav = ({socials}) => {
  return (
    <Box sx={{ display: { xs: 'none', md: 'flex' }, flexDirection: 'column', position: 'fixed', bottom: '0', left:'1.1rem', alignItems: 'center' }}>
...
const Icon = ({ site, label,link }) => {
    ...
    return (
        <Box
            sx={[{ mb:1,cursor: 'pointer', transform: 'translateY(0)', transition: 'all 200ms ease-in-out' },
            (theme) => ({
                '&:hover': {
